export const meta = {
  ...
  description: 'IES Plan 核心规格并行设计: DB schema / 物理财务模型 / 任务调度 / 注册表与诊断',
  phases: [
...
const DESIGNERS = [
  {
    ...
    prompt: `你是 IES Plan 的数据架构师。设计 PostgreSQL 权威事实的完整关系模式。

## 系统背景
IES Plan 是综合能源系统(电/热/冷)规划软件。核心理念(必须遵循):
...
你必须实际把文档写入磁盘(用 Write 工具)。`,
  },
  {
    ...
    prompt: `你是 IES Plan 的能源系统数学建模专家。设计计算引擎的数学模型规格。
...
完整中文 Markdown 规格文档(数学公式用 LaTeX 格式,代码块中给关键算法的伪代码),写到 docs/spec/02-calc-model.md。必须覆盖以上全部 10 点,可直接作为求解器适配层的实现依据。用 Write 工具实际写入磁盘。`,
  },
  {
    ...
    prompt: `你是 IES Plan 的任务调度与分布式一致性架构师。设计任务状态机与调度协议。
...
完整中文规格文档(含状态机图 mermaid、租约时序图 mermaid、协议字段表),写到 docs/spec/03-task-scheduling.md。用 Write 工具实际写入磁盘。`,
  },
  {
    ...
    prompt: `你是 IES Plan 的受控扩展与诊断体系设计师。设计注册表、单位系统与诊断目录。

## 系统背景
IES Plan 综合能源规划软件。约束:
...
const synthesis = await agent(
  `你负责 IES Plan 设计阶段的综合审查。下面 4 份设计规格文档已经写入磁盘(它们是并行产出的,可能存在术语不一致、接口不一致或冲突):
