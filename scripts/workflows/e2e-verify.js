...
const e2e = await agent(
  `你是 IES Plan 的全栈端到端验证者。工作目录 /home/mc/Documents/工作文档/IES_Plan。
...
- 若 e2e 脚本不方便 curl 手打, 可写 /home/mc/Documents/工作文档/IES_Plan/backend/tests/e2e_full.py 用 httpx 连 http://localhost:8080 跑完整流程(放 tests/ 但不要求 pytest 默认收集, 或标记 @pytest.mark.e2e)
- 中文输出最终报告`,
...
const accept = await agent(
  `你是 IES Plan 的验收审查者。工作目录 /home/mc/Documents/工作文档/IES_Plan。

全栈已构建并完成端到端冒烟验证。现在对照设计输入做最终验收与补漏。

## 步骤
1. 阅读 /home/mc/Documents/工作文档/IES_Plan/IES-Plan-Design-Input.rpd 的 17.9 节(编号化产品约束目录)与 18 节(产品完成边界 AC-MVP-001~008)
