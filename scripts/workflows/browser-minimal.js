...
function browserPrompt(extra) {
  return `你是 IES Plan 的浏览器端到端测试员。使用 browser-harness CLI(命令名 browser-harness, 不是 browser-use)在真实 Chrome 中执行最小案例完整闭环, 发现并报告所有 bug。
...
function fixPrompt(bugs, target) {
  ...
  return `你是 IES Plan 的修复工程师。修复以下浏览器测试发现的 ${target} 侧 bug。工作目录 /home/mc/Documents/工作文档/IES_Plan。
...
while (bugs.length && round < 3) {
  ...
  const verify = await agent(
    `你是 IES Plan 的部署与回归验证员。

## 步骤
1. 重建并重启全栈: cd /home/mc/Documents/工作文档/IES_Plan && docker compose build web backend 2>&1 | tail -3 && docker compose up -d web backend worker io_worker 2>&1 | tail -5
