...
9.涉及到较复杂的开发工作，尽量使用herdr拉起claude让子进程完成，简单的修改合并可以自己完成。
10.只实现当前明确的核心业务逻辑，不得针对未提出、无证据的攻击面或故障模式增加假想安全加固、备用分支或防御性代码；及时删除未使用的防御代码，避免过度工程。
11.校验只保留用户输入、用户自定义能力公开输出及明确核心业务规则所必需的部分。不得在内部文件交换、读取、持久化、快照、运行前后或可信流程中新增重复校验、完整性复核、摘要/hash 比对、冗余诊断或仅为假想风险准备的防御分支；应删除已有的非必要校验及其文档和测试。
