...
> 版本: 0.1(草案)
> 状态: 设计规格
> 适用范围: pIES 综合能源规划软件(任务调度器 / 计算 Worker / I/O Worker / 并发槽 / 租约协议)
...
本文档定义 pIES 的**任务调度与分布式一致性协议**:任务从提交、排队、领取、执行、取消、超时、失败到终态的完整生命周期,以及多 Worker 并发执行时的权威事实、租约与 fencing token 规则。
