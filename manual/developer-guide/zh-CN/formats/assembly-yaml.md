...
connections:
  ...
  pv_to_grid:
    from: pv_1.electricity_out
    to: grid.electricity_in
...
示例中的摘要仅为占位值。正式文件必须引用设备规范内容与有效财务快照的真实 SHA-256：`ref.sha256` 为 Effective 文件的**对象完整字节摘要**（文件含自身 `content_sha256` 字段），血缘区 `profile_sha256`/`overrides_sha256`/`content_sha256` 为**规范内容摘要**，两者分别验证、不要求相等（规则见下文「规划配置与有效财务快照引用」）。被引用的有效快照需含本装配用到的 `finance_type`（如 `pv_system`）与 `energy_prices` 的 `price_id`（如 `grid_import`/`pv_export`），完整字段定义见[财务 YAML 契约](finance-yaml.md)。实例 `instance_id`（如 `pv_1`/`grid`）、计量绑定 `binding_id`（如 `import_meter`）、财务 `finance_type`（如 `pv_system`）与技术模型 `device.id`（如 `acme.device.pv`）严格分离：`grid` 无设备成本、不绑定 `finance_type`（不再虚构 `grid_connection` 等零成本连接类别），其计量与计费完全由 `tariff_bindings` 表达。
...
| `finance` | 对装配前由 `FinanceProfile` + `FinanceOverrides` 合并生成、完整校验的不可变 `EffectiveFinanceConfig` 的**精确引用**：`ref` 携带对象完整字节摘要，血缘区携带 `profile_id` / `profile_sha256` / `overrides_sha256` / `content_sha256` 规范内容摘要（两类摘要分别验证）；规划与财务计算的共同消费与证据；装配内不内联另一份完整财务配置 |
...
引用与血缘规则（对象字节摘要与规范内容摘要分别验证，两类摘要的定义与关系见[财务 YAML 契约](finance-yaml.md)「规范化与摘要」）：

- `ref.sha256` / `object_id` 是**对象字节摘要**：对 Effective YAML 文件完整字节（含其自身 `content_sha256` 字段）计算。装配读取被引用对象后，必须先对完整字节重算 SHA-256 并等于 `ref.sha256`；不匹配拒绝；
- `content_sha256` 是**规范内容摘要**：解析对象后按财务规范化算法重算（移除派生摘要字段 → 规范 YAML 字节 → SHA-256），必须同时等于 `finance.content_sha256` 与被引用 Effective 文件自身的 `content_sha256` 字段。两类摘要互不要求相等（文件携带自身 `content_sha256` 字段，字节摘要通常不等于内容摘要），缺一不可；
- 导入时把精确 Profile、Overrides 与 Effective 一起重新合并验证（`profile_ref`、`overrides_sha256`、重算 `content_sha256` 全部一致），不匹配阻断且不静默取 Profile 或任意来源；
...
finance_binding:
  instances:
    pv_1:
      ...
      drivers:
        capacity_kw:
          property: rated_power                    # property 只对应 scalar（映射可省略 aggregation）
        generated_kwh:
          interface: electricity_out               # 时序 interface 必须写 aggregation
          aggregation: integrate_positive
    grid:
...
- 聚合语义：`property` 只对应 `scalar`，映射 `property` 时可省略 `aggregation`（语义固定为 `scalar`）；时序 `interface` 必须配 `integrate_positive`（非负功率/流率按 `step_hours` 积分）或 `sum_positive`（区间累计接口直接作为能量序列）；本契约不接受模糊双向净值/绝对值，负值序列在聚合前阻断；
...
- 载体与方向相容性校验：价格 `carrier` 必须与绑定接口的载体相容（同一公共载体词汇元素；载体取值与规则见[设备模型 YAML](device-model-yaml.md)「interfaces」，本格式页不复制清单）；`direction` 必须与接口流向语义一致——`purchase` 计量的能量由公共设施/外部经该接口流入本站（示例：`grid.electricity_out`，从计量点流向负荷侧），`sale` 计量的能量由本站生产经该接口流向公共设施/外部（示例：`grid.electricity_in` 接收站内馈入）；流向不符或需猜测的绑定阻断；
...
1. **结构校验**：安全 YAML、schema、字段类型、ID、内容摘要和引用形状（含 `finance` 精确引用与三摘要血缘、`finance_binding` 判别联合、`tariff_bindings` 的 binding_id 结构与命名规则）；
...
生成器必须同时验证三者。人工修改规范文本、替换资源或变更财务快照/绑定后，摘要和回执失效，必须重新装配。任务创建后只消费该产物，不得重新读取“当前设备”“当前价格”或“最新项目”。
...
- 五类 interface、三类 predefined 来源、存量/新增、设备成本绑定（`costed`/`none`）、按增量语义的 driver 齐全性、`tariff_bindings` 的 `price_id`/`carrier`/`direction` 校验（含 `type: none` 计量点）、财务分量标识符映射（实例前缀与绑定前缀的命名空间规则）与带单位常量量纲校验、规划目标逐项引用与有效财务快照血缘（对象字节摘要与内容摘要分别验证）均有契约测试；
