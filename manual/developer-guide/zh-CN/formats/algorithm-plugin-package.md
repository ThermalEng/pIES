...
- 在取消、超时或租约失效时终止完整进程组；
- 生成带 runner 版本、实际限制、退出状态和输出的执行回执。
...
- 同一精确版本的包字节不可替换；
- 共享园地审批绑定插件 ID 和版本；批准后上传同名新包不能继承批准状态；
- 用户安装共享插件只建立目录引用，不复制 ZIP、校验报告或环境缓存；
- 历史任务始终引用原包和环境，不自动迁移到新版本。

## 完成标准

- 合法最小包可以在无网络 Docker 隔离环境完成加工、求解和结果校验；
- 非法路径、压缩炸弹、错误入口、越权文件、网络访问和资源超限均被稳定诊断阻断；
- API 和普通 Worker 测试证明不会 import 或执行包内源码；
- 相同输入和确定性插件产生相同加工文件与结果基线；
