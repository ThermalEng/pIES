...
项目计算不建立时区语义。项目创建时固定时间分辨率、是否考虑闰年和单场景模式；计算序列使用从零开始的连续 `step`，并声明分辨率、点数和单位。原始输入序列至少覆盖一个完整年度，点数为项目基线年度点数的正整数倍；同一项目的所有输入序列必须与项目基线分辨率一致，且彼此点数和 `step` 一致。装配前不做重采样、插值、聚合、融合或补齐；按项目基线生成全周期计算序列属于计算阶段。
