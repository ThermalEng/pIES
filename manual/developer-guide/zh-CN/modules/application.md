...
输入是项目 ID、项目 revision 和项目模型实例的预定义来源。该用例不生成未来全周期序列，也不选择预测算法：

1. 授权并固定项目计算基线摘要以及相关模型实例 revision；
2. 枚举 `predefined` interfaces，并分别校验 `constant/data_repeat/data_predict` 来源声明；
3. 校验数据文件、单位、完整年度、分辨率、点数和 `step`，任一序列不一致时阻断，不做重采样、插值、聚合、融合或补齐；
4. 对 `data_predict` 校验目标类型、训练目标、历史输入和所需未来已知协变量；
5. 将规范数据、变换回执和来源声明作为同一用例的不可变内容寻址引用提交；
