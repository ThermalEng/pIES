...
`assembly` 是从可编辑业务状态进入计算世界的唯一同步闸门。它把项目设备、真实序列接口、连接、数据绑定、规划经济配置、约束和计算选择整理为标准装配 YAML，证明这份输入完整、引用稳定且具备所选计算能力，然后签发不可变 `ValidatedAssemblyArtifact`。
...
- 校验结构、内容摘要、接口类型、载能、量纲、单位、预定义数据、方程、规划经济配置和整体可解性；
...
- 设备内容摘要、方程 contract、generator、solver、规划经济配置和所有资源形成完整依赖锁。

相同语义必须产生相同规范文本和摘要。规范化算法变化必须版本化；历史 artifact 不能用当前算法悄悄重算后覆盖。

## 增加装配字段或规则

1. 确认字段的唯一所有者；若属于设备模型、CSV、方程 contract、规划经济配置或 generator 选项，先修改其权威 contract；
...
- 所有设备内容摘要、方程 contract、generator、solver、schema、经济配置和资源版本进入依赖锁；
- 参与摘要的顺序和序列化唯一；
