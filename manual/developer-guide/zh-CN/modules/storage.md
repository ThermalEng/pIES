...
`storage` 统一管理数据文件、逐时结果、证据和导出包等大型不可变内容。它把业务对象与文件系统/S3 等具体实现隔开，并保证引用和清理生命周期可解释。
...
- BlobStore provider 和对明确中断操作的幂等收敛；
- 本模块健康状态。
...
`ObjectHandle` 至少包含对象 ID、大小和媒体类型。其他公开输出包括 owner 引用和对明确中断操作的恢复结果。
...
数据库与 BlobStore 不能伪装成一个 ACID 事务。已明确发生中断时，重试必须通过操作 ID 识别已完成部分并幂等收敛；正常读写不扫描或复核其他对象。
...
5. 测试已观测写入中断后的幂等重试与临时对象清理；
6. 由组合根选择 provider，业务代码不增加 provider 分支。

## 失败语义

- 实际写入失败：返回原始存储错误，不伪造成功 handle；
- 对象缺失：返回明确缺失，不回退同名旧文件；
- attach/detach 重复：按公开幂等语义处理；
- 已记录的写入操作中断：按操作 ID 幂等重试或清理本次临时对象；
