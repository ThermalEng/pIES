...
- 把规划经济配置形成系统目标项，但不把价格或成本写回设备技术语义；
- 将规范业务单位显式转换为求解器内部单位；
...
| 规划经济与计算配置 | 存量/新增身份、投资/O&M/购售价格、精度和算法选项均已校验 |
...
方程贡献提供声明式变量、技术约束、状态关系、接口流和结果映射元数据，不直接操作具体 solver 私有对象，不写文件，不运行进程。投资、运维和能源价格形成的目标项由 generator 从规划经济配置加入；计算精度也由计算配置决定。
