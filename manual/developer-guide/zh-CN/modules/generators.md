...
方程贡献提供声明式变量、技术约束、状态关系、接口流和结果映射元数据，不直接操作具体 solver 私有对象，不写文件，不运行进程。目标及规划边界来自规划配置，投资、运维、能源价格、税率和资金时间成本等共用经济参数来自公共财务配置；预测算法、计算精度及求解选项由 `CalculationConfig` 决定。生成器不训练预测模型；它只消费当前阶段已物化的序列，因此阶段二每次重建后可独立重新生成 Bundle。
