...
> 文档状态：生效目标契约；设备模型、设备数据与装配目标 schema 为 `2.0.0`，Solver Bundle 目标 schema 为 `1.0.0`；实现迁移见 [Roadmap](../../changelog/roadmap.md)。

本章规定插件开发和离线交换必须共同遵守的文件边界。目标不是把内部对象“导出成差不多能读的文件”，而是让开发者只看本章及对应格式页，就能手写、校验和交付一个不依赖当前代码目录的输入包。

当前代码中的目录 YAML、CSV 和装配对象仍是迁移输入，不自动等同于本标准。实现完成前，不能仅凭文件扩展名宣称兼容本页所列目标 schema。
