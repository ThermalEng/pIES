...
> 文档类型：实施控制文件；目标版本：`1.1.0`；功能状态：未实现；产品蓝图：[模型与算法](../customization-center.md)
...
2. [模型与算法](../customization-center.md)；
3. [设备模型 YAML](../formats/device-model-yaml.md)与[算法插件包](../formats/algorithm-plugin-package.md)；
4. 本切片涉及的模块手册；
...
- 模型可从网页参数配置、在线 YAML 编辑或上传 YAML 创建，三者必须汇合为同一草稿与规范版本；
...
- 所有者可以申请共享某个精确版本，只有管理员批准后才进入共享园地；
...
| 路由 | 页面 | 主要动作 |
|---|---|---|
| `/customizations/models` | 模型库 | 新建、表单/YAML 编辑、上传、校验、发布版本、申请共享、停用 |
| `/customizations/plugins` | 算法插件 | 上传 ZIP、查看校验/环境/自检、申请共享、停用 |
| `/customizations/shared` | 共享园地 | 搜索、筛选、查看精确版本、安装/已安装状态 |

一级导航点击“模型与算法”默认进入 `/customizations/models`。三个入口在桌面端使用可见页签或侧栏，在移动端使用可键盘操作的折叠导航；URL 必须可深链接和刷新恢复。
...
目标用例目录为 `backend/iesplan/application/customizations/`。它负责编排目录、草稿、版本、共享、安装、审计和对象引用，但不自行实现设备 YAML 语义或插件运行。

职责分配：

| 代码边界 | 必须负责 | 禁止负责 |
|---|---|---|
| `devices` | YAML 安全解析、schema、规范化、descriptor 和命令引用校验 | 用户权限、共享审批 |
...
- 首次创建时为用户分配不含个人信息的公开随机命名空间，稳定 ID 使用 `user.<public_namespace>.<kind>.<slug>`；
...
- `user_id`、`custom_version_id`、来源 `owned|shared_garden`；
- 安装/卸载时间和状态；
- 活跃 `(user_id, custom_version_id)` 唯一；
- 安装事务中 attach storage owner，卸载事务中 detach；
- 禁止保存 YAML、ZIP、报告或依赖副本。

### `share_requests`

- `custom_version_id`、申请者、申请摘要、许可证、公开说明；
- 包/模型摘要快照；
...
draft(revision N)
  → validate
  → invalid（保留诊断和草稿）
  → private_ready version（不可变，所有者自动安装）
```
...
批准绑定精确 version + content hash。新版本重新申请。管理员不能批准校验失败或许可证缺失的版本。
...
| `GET /api/customizations/models/{id}` | 条目、草稿、版本、安装与共享状态 | `{model, draft, versions}` |
...
| `POST /api/customizations/models/{id}/versions` | 校验并发布精确版本 | 201 `{version, installation, report}` |
...
| `GET /api/customizations/plugins/{id}` | 版本、报告、环境、安装与共享状态 | `{plugin, versions, reports}` |
...
| 方法与路径 | 语义 | 成功 |
|---|---|---|
| `POST /api/customizations/versions/{id}/share-requests` | 所有者申请共享精确版本 | 201 `{share_request}` |
| `POST /api/customizations/share-requests/{id}/withdraw` | 所有者撤回待审申请 | `{share_request}` |
| `GET /api/shared-garden` | 浏览已批准精确版本 | `{items, next_cursor}` |
| `GET /api/shared-garden/{version_id}` | 公开详情、报告摘要和安装状态 | `{item, installation}` |
| `POST /api/shared-garden/{version_id}/install` | 幂等建立当前用户目录引用 | 201/200 `{installation}` |
...
| `POST /api/admin/share-requests/{id}/approve` | 批准精确版本共享 | `{share_request}` |
| `POST /api/admin/share-requests/{id}/reject` | 驳回并记录说明 | `{share_request}` |
| `POST /api/admin/shared-versions/{id}/unshare` | 停止新浏览/安装 | `{version, share_status}` |
| `POST /api/admin/custom-versions/{id}/block` | 安全阻断新任务 | `{version, blocked}` |
...
选择目录响应必须包含 `source(system|owned|shared_garden)`、稳定 ID、精确版本、摘要、能力、兼容性和 `selectable`/不可选诊断。不得要求前端分别请求注册表、用户目录和共享园地后自行合并。

所有端点先登记允许的一级包装键。上传大小/配额用 413，业务内容校验用 400，请求 schema 用 422，revision/同版本不同内容/状态冲突用 409，权限用 403。卸载确认令牌必须绑定 installation、影响清单、操作者和短期过期时间，清单变化后拒绝执行。当前架构宪法没有 `CUSTOM` 诊断域；未经用户批准修改宪法前，应使用现有 `API`、`CONFIG`、`TASK`、`OBJ`、`PERM` 等权威域，不得私自发明不合规前缀。
...
- 发布对话框要求精确版本和变更说明，并显示摘要与依赖锁；
- 发布成功后草稿不被静默清空，可选择从该版本派生下一草稿；
- 同名显示名允许，稳定 ID + 版本必须唯一；
...
- 新增 CustomEntry/Version/Installation/ShareRequest 应用 contract；
...
退出：100 个用户安装同一版本仍只有一个内容对象，引用与清理正确。

### 切片 3：模型库后端

- 模型条目、草稿 revision、上传、校验和版本发布用例/API；
- 复用 devices 唯一 YAML 解析与规范化器；
- 所有者发布后自动创建安装引用。

退出：表单等价 YAML 与上传 YAML 产生相同规范摘要；竞态发布被 409 阻断。

### 切片 4：模型库前端

- 一级导航、三个入口骨架；
- 模型列表、表单、YAML 编辑、上传、诊断、发布和版本详情；
...
- 后端统一设备模型 catalog；
- 画布设备目录消费系统 + 当前用户安装模型；
- 保存稳定 ID 与精确版本，装配和项目快照固定摘要。

退出：未安装/不可用模型不可选择；参数配置浏览器验收通过；拖放人工验收。
...
退出：共享版本安装后进入对应选择器；取消共享不破坏既有安装；存储不随安装人数增长。
...
- 项目保存显示名、列表下标或 `latest`，未固定精确版本和摘要；
