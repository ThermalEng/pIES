...
对程序在同一可信流程内本地生成并继续使用的文件、数据、中间产物和派生配置，禁止重算并比对 SHA-256/hash 进行额外自校验，也不为此增加测试。这类产物的正确性由生成函数的领域校验、类型契约和事务边界保证。

Hash 只在对象存储内容寻址，上传、导入、下载、备份或外部运行器交付的字节完整性，以及现行公开契约明确使用内容身份固定不可变发布物或精确引用时保留。保留必须指向具体边界或契约，不得以泛化的“安全”、“防篡改”或“可能损坏”为由。
