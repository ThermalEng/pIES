...
对文本/声明式文件（设备模型 YAML、设备数据 CSV、装配 YAML、财务 YAML、规划/计算配置 YAML、Solver Bundle 描述）只校验字头（`schema`/`schema_version`）与领域约束，不做 SHA-256/内容摘要自校验，也不为此增加测试。这类产物的正确性由字头、类型契约、领域校验和事务边界保证。

SHA-256/哈希只在二进制对象存储内容寻址、上传/导入/下载/备份的二进制字节完整性，以及外部运行器交付的二进制产物（算法插件 ZIP、求解器原始输出、证据包字节）时保留。文本文件不得以“安全”“防篡改”或“可能损坏”为由增加哈希校验。
...
- 规范化算法必须唯一并由公开纯函数实现；
- `schema_version`、创建来源和依赖版本必须共同保存；二进制对象另保存哈希；
