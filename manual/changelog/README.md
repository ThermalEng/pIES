...
- 静态架构门禁通过；门禁只保留当前检查与稳定允许项说明，检查语义不变。

### 2026-09-13 — `0.8` 开发前，后端解耦第一阶段收口

**完成时间**：2026-09-13

**版本进展**：完成后端解耦第一阶段（旧依赖消除）；完整依赖方向收敛见本页顶部“后端依赖架构收口”条目；不改变产品版本，下一开发目标仍为 `0.8.0`。
...
- application 直调旧 service、裸表穿透、Worker 持有事务、双实现并存、计算失败被租约错误遮蔽等偏离全部消除；`0.8` 计算只需实现新计算边界，不再穿透旧代码。
