        ...
        if not sha:
            diags.append(
                make_diag(
                    ...
                    location={"object_type": "assembly", "field": f"resources.datasets.ds{vid}.source.sha256"},
                )
            )
            continue
...
def _build_calculation(
    ...
    gen = generator or str(calc_cfg.get("generator") or calc_cfg.get("algorithm") or "ies.algo.milp_hybrid@1.0.0")
    sol = solver or str(calc_cfg.get("solver") or LEGACY_SOLVER_REF)
