...
def transport_pipe(
    ...
    """传输管道: 同一步能量损失 out[t] = in[t] * (1 - loss_rate)。

    管道真实语义只有同一步能量损失，关系由 loss_rate 显式表达；loss_rate=0 即无损直接关系。
    不含历史状态移位。
    ...
    if not np.all(np.isfinite(in_arr)):
        raise ValueError("in_flow_w 包含 NaN/Inf")
    return in_arr * (1.0 - loss_rate)
