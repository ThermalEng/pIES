"""审计服务: audit_log 唯一写入入口与查询。

依据 架构宪法 §16 安全与审计 + 领域模型 §身份、权限和审计 + 领域模型 §对象生命周期(audit_log 不可变):

- audit(): 统一审计入口, 只保存身份/时间/动作/对象标识/修订号/结果/必要校验信息,
  不复制密码、令牌、完整模型、完整数据集或原始求解日志(宪法 §16);
...
audit_log 不可变(领域模型 §对象生命周期): 本模块只 INSERT, 不提供任何修改/删除路径;
审计事件与业务写入同事务提交, 事务边界由 API 层(请求级)控制。
"""
