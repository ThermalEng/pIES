...
行级读写全部经 application.worker 用例(db 会话 + id/参数进, 记录/id 出),
...
def renew_lease(db: Session, attempt_id: int, token: UUID) -> bool:
    ...
    """
    n = worker_app.renew_lease_once(db, attempt_id, token, ttl_seconds=LEASE_TTL_SECONDS)
    if n == 1:
...
def submit_result(
    ...
    *,
    ...
    if snapshot is not None:
        ...
        evidence_id = worker_app.create_evidence_record(
            db, task_id=task.id, attempt_id=attempt_id, snapshot_id=snapshot.id,
            object_id=object_id, created_by=who,
        )
        worker_app.attach_result_ref(db, object_id, evidence_id, actor_id=who)

        assessment = payload.get("assessment") or {}
        assessment_id = worker_app.create_assessment_record(
            db,
        ...
        worker_app.flip_result_index(db, project_version_id=snapshot.project_version_id)
        index_id = worker_app.insert_result_index(
            db,
...
def release_attempt(
    ...
    n = worker_app.fence_release_lease(db, attempt_id, token, status=lease_status)
    if n != 1:
        raise LeaseRejectedError(
            "租约失效, 尝试收尾被拒绝",
            params={"attempt_id": attempt_id},
        )
    attempt = worker_app.get_attempt_record(db, attempt_id)
    ...
    finished = worker_app.finish_attempt_record(
...
def cancel_attempt(
    ...
    cancelled = worker_app.cancel_task_record(db, task.id, outcome=outcome)
