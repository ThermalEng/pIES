...
S7 收口: 端点级完整用例(登录/注册/用户列表/账号状态变更/删除预告确认/
安全设置/OIDC 回调/会话校验)全部下收至此, API 只留请求 DTO、
凭证提取/Cookie/重定向传输与错误/响应映射。
"""
...
# ---------------------------------------------------------------------------
# S7 收口: 端点级完整用例(原 api/auth.py 的多能力编排整体下收, API 只留
# 请求 DTO、凭证提取/Cookie/重定向传输与错误/响应映射)
# ---------------------------------------------------------------------------
