...
def result_view(
    ...
    """结果视图(只读转调)。"""
    return results_service.result_view(db, user, project_id, task_id)


def list_assessments(db: Session, task_id: int) -> list[ResultAssessmentRecord]:
    """任务评估历史(只读转调)。"""
    return results_service.list_assessments(db, task_id)


def assessment_to_dict(db: Session, assessment: ResultAssessmentRecord) -> dict[str, Any]:
    """评估记录序列化(只读转调)。"""
    return results_service.assessment_to_dict(db, assessment)


def latest_evidence(db: Session, task_id: int):
    """任务最新证据包(只读转调)。"""
    return results_service.latest_evidence(db, task_id)


def evidence_content(db: Session, package) -> dict[str, Any]:
    """证据包内容(只读转调)。"""
    return results_service.evidence_content(db, package)
...
def select_task_result(
    ...
    try:
        ...
        db.rollback()
        raise
    return selection, results_service.selection_diff(db, project_id)
...
def create_check_task(
    ...
    """创建检查任务(report 类型; 提交由本用例拥有)。"""
    try:
        task = results_service.run_check_task(
