"""数据集 CSV 解析与校验纯函数(Wave 2 W2-A: application/datasets)。

从 ``iesplan.services.dataset`` 复制的无副作用纯函数（模板生成 / CSV 解析 /
时间轴与数值校验 / 质量报告），旧服务保留未删。复制内容不做任何语义改动；
本模块不访问数据库与对象存储（调用方向：application/datasets/lifecycle
消费本模块 + dataset/project/identity/storage 域公开门面）。
"""
