...
初始化先由当前 ORM 定义建立基础 schema；版本化 migration 负责明确登记并
补充各版本拥有的当前结构与约束，所有迁移在同一事务中幂等执行。
...
迁移 DDL 按方言分支提供(SQLite 测试 / PostgreSQL 生产), 与当前 ORM 模型
语义一致；ORM 模型定义基础 schema，版本化迁移负责后续结构变更。
"""
...
# ---------------------------------------------------------------------------
# 迁移 0002: 用户模型模板主表与不可变发布 revision 表
# ---------------------------------------------------------------------------
