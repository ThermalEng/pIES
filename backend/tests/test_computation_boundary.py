"""computation 边界契约测试(Wave4-A, F3 接线后)。
...
- 宪法 §4.5 方法形状: generate 消费装配产物/已固定资源/公开配置产出
  ...
  统一结果;
- 深层不可变: Bundle/回执/结果/配置的嵌套容器只读, 调用方改写不污染;
...
def test_config_schema_word_checked() -> None:
    ...
    good = CalculationConfig.from_snapshot(
        {"params": {"gap_rel": 0.01}}, seed=42, tolerances={"rel": 1e-6},
    )
    assert good.seed == 42
    assert good.to_dict()["options"] == {
        "gap_rel": 0.01, "tolerances": {"rel": 1e-6},
    }
    with pytest.raises(ValueError, match="params"):
        CalculationConfig.from_snapshot({"params": [1]}, seed=None, tolerances=None)
