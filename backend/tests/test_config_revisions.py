...
覆盖:
- Profile 登记: 内容寻址去重幂等、region 元数据;
- 项目引用 Profile: 原子生成空覆盖 Effective(用户不可直接 author Effective);
- Overrides 保存: 追加不可变 revision → 确定性重合并生成 Effective;
...
- 篡改: 声明 content_sha256 与重算值不一致拒绝;
...
def test_profile_registration_roundtrip_and_dedup(client: TestClient, db_session: Session) -> None:
    ...
    assert profile.content_sha256 == profile.computed_sha256()
...
def test_profile_registration_rejects_tampered_sha(client: TestClient, db_session: Session) -> None:
    headers, _ = _owner(client, db_session)
    bad = {**PROFILE_PAYLOAD, "content_sha256": "0" * 64}
    resp = client.post("/api/finance-profiles", json={"finance_profile": bad}, headers=headers)
...
def test_project_set_profile_generates_effective(client: TestClient, db_session: Session) -> None:
    ...
    assert effective.content_sha256 == effective.computed_sha256()
    # GET 一致
...
def test_profile_switch_invalidates_chain(client: TestClient, db_session: Session) -> None:
    ...
    resp = client.put(
        f"/api/projects/{pid}/finance-profile",
        json={"finance_profile": profile2_payload},
        headers=headers,
    )
    assert resp.status_code == 200
