...
class TestUnhappyPath:
    ...
    def test_nonfinite_option_blocks(self, init_registry):
        ...
        result = validate_assembly_doc(
            parsed.doc, package_dir=SAMPLES_DIR, datasets=SAMPLE_DATASETS
        )
        assert result.artifact is None
...
class TestProjectExport:
    def test_project_export_issues_artifact(self, init_registry):
        content = {
            ...
            "model": {
                "devices": [
                    ...
                    {
                        "id": 2, "device_type": "ies.device.heat_pump", "kind": "new",
                        "params": {"type_detail": "ies.device.heat_pump",
                                   "rated_heat_kw": 600, "cop": 3.5, "cop_profile": 0},
                    },
                    {
                        "id": 3, "device_type": "ies.device.electric_load", "kind": "existing",
                ...
                "ports": [
                    {"id": 11, "device_id": 1, "name": "electric_out", "port_type": "electric", "direction": "out"},
                    {"id": 21, "device_id": 2, "name": "electric_in", "port_type": "electric", "direction": "in"},
                    {"id": 22, "device_id": 2, "name": "heat_out", "port_type": "thermal", "direction": "out"},
                    {"id": 31, "device_id": 3, "name": "electric_in", "port_type": "electric", "direction": "in"},
                ],
                "connections": [
        ...
        datasets = {
            17: {
                "columns": ["e_load"], "column_units": {"e_load": "kWh"},
                "resolution": "1h",
    ...
    def test_project_export_loss_rate_wraps_pipe(self, init_registry):
        content = {
            ...
            "model": {
                "devices": [
                    {"id": 1, "device_type": "ies.device.grid_connection", "kind": "existing",
                     "params": {"type_detail": "ies.device.grid_connection", "max_import_power_kw": 800, "max_export_power_kw": 0}},
                    {"id": 2, "device_type": "ies.device.heat_pump", "kind": "new",
                     "params": {"type_detail": "ies.device.heat_pump", "rated_heat_kw": 600, "cop": 3.5, "cop_profile": 0}},
                    {"id": 3, "device_type": "ies.device.heat_load", "kind": "existing",
                     "params": {"type_detail": "ies.device.heat_load", "heat_profile": {"dataset_version_id": 18, "unit": "kW"}}},
                ],
                "ports": [
                    {"id": 11, "device_id": 1, "name": "electric_out", "port_type": "electric", "direction": "out"},
                    {"id": 21, "device_id": 2, "name": "electric_in", "port_type": "electric", "direction": "in"},
                ...
                "connections": [
                    {"id": 101, "from_port_id": 11, "to_port_id": 21, "loss_rate": 0},
                    {"id": 102, "from_port_id": 22, "to_port_id": 31, "loss_rate": 0.05,
        ...
        datasets = {
            18: {
                "columns": ["h_load"], "column_units": {"h_load": "kWh"},
                "resolution": "1h",
                "sha256": hashlib.sha256(b"x").hexdigest(),
                "media_type": "text/csv",
            },
    ...
    def test_project_export_missing_dataset_sha_blocks(self, init_registry):
        ...
        assert result.artifact is None
        assert any(
            d.params.get("reason") == "dataset_sha256_required" for d in result.diagnostics
        )
    ...
    def test_artifact_corrupted_canonical_text_raises(self, init_registry):
        ...
        from iesplan.assembly.contracts import ValidationReceipt, ValidatedAssemblyArtifact
